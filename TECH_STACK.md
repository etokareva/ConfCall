...
Не библиотека, а доменное решение проекта. Участник группы создаёт ссылку на `/book` после выбора группы и нужных участников. В `/booking-links` пользователь видит только собственные созданные ссылки; в таблице доступны копирование URL и удаление. Гость без аккаунта видит общую доступность выбранных участников и бронирует свободный слот.
