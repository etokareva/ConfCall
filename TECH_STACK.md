...
Все публичные selectors компонентов и UI-директив используют проектный префикс `ccs`: `<ccs-navbar>`, `<ccs-user-select>`, `ccsTooltip`. Префикс `app` не используется для новых Angular selectors.
