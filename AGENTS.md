...
- В `/booking-links` отображаются только ссылки, созданные текущим пользователем. В последней колонке таблицы оставлять только действия `copy` и `delete`; не возвращать `open`, `send`, `toggle` в action-колонку без отдельного UX-решения. Не возвращать список публичных ссылок в stepper `/book`.
...
- Страница `/booking-links` должна оставаться лаконичной: URL должен быть кликабельной ссылкой в основной колонке, а действия со ссылкой — только иконки копирования и удаления с tooltip/aria-label.
